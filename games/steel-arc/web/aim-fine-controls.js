...
export function createFineAimControls({container,getValues,onChange}){
  ...
  function update({heading,power}=getValues()){
    const force=Math.round(power);root.querySelector('[data-fine="power"] [data-step="-1"]').disabled=force<=20;root.querySelector('[data-fine="power"] [data-step="1"]').disabled=force>=100;
  }

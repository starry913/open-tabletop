...
test('host selects a shared duel map, random remains available, and rematches use the current choice',async()=>{
  ...
  const guest=await service.request(code,'','join',{name:'Guest',seatKey:seatKey(91)});
  assert.equal(owner.room.mapChoice,'random');
  await assert.rejects(()=>service.request(code,guest.token,'map',{mapChoice:'alpine'}),{status:403});
  await assert.rejects(()=>service.request(code,owner.token,'map',{mapChoice:'range'}),{status:400});
  await assert.rejects(()=>service.request(code,owner.token,'map',{mapChoice:'unknown'}),{status:400});
  await service.request(code,owner.token,'map',{mapChoice:'alpine'});
  assert.equal((await service.request(code,guest.token,'state')).room.mapChoice,'alpine');
  await ready(service,code,guest);
  const first=await service.request(code,owner.token,'start');
  assert.equal(first.game.battleTerrain.themeId,'alpine');
  await assert.rejects(()=>service.request(code,owner.token,'map',{mapChoice:'bay'}),{status:409});
  const stored=await service.store.get(code,Date.now());
  stored.room.status='finished';stored.room.engine.phase='ended';
  await service.store.cas(code,stored.revision,stored.room,stored.expiresAt);
  await service.request(code,owner.token,'map',{mapChoice:'bay'});
  await service.request(code,owner.token,'rematch');await service.request(code,guest.token,'rematch');
  const second=await service.request(code,owner.token,'start');
  assert.equal(second.game.battleTerrain.themeId,'bay');
  const finished=await service.store.get(code,Date.now());
  finished.room.status='finished';finished.room.engine.phase='ended';
  await service.store.cas(code,finished.revision,finished.room,finished.expiresAt);
  await service.request(code,owner.token,'map',{mapChoice:'random'});
  await service.request(code,owner.token,'rematch');await service.request(code,guest.token,'rematch');
  const random=await service.request(code,owner.token,'start');
  assert.ok(BIOME_IDS.includes(random.game.battleTerrain.themeId));
});
